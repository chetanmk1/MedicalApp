...
async function run() {
  ...
  try {
    ...
    // 3. Create Clinic (Super Admin)
    logTest('3. Testing Clinic Onboarding...');
    ...
    if (clinicRes.status !== 201) throw new Error(`Clinic creation failed: ${JSON.stringify(clinicData)}`);
    const clinicId = clinicData.clinic._id;
    logSuccess(`Clinic Onboarded: ${clinicData.clinic.name} (${clinicId})`);

    // 4. Create Clinic Admin
    logTest('4. Testing Clinic Admin Account Creation...');
    ...
    if (adminRes.status !== 201) throw new Error(`Admin creation failed: ${JSON.stringify(adminData)}`);
    logSuccess('Clinic Admin account created');
    ...
    logTest('13. Testing Cancellation...');
    const cancelRes = await fetch(`${BASE_URL}/appointments/${appId}/cancel`, {
      method: 'PATCH',
      headers: { 'Authorization': `Bearer ${patientToken}` }
    });
    if (cancelRes.status !== 200) throw new Error('Appointment cancellation failed');
