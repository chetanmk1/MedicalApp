...
// @desc    Create a user (doctor, staff by clinic_admin; clinic_admin by super_admin)
...
export const createUser = async (req, res) => {
  try {
    ...
    if (req.user.role === 'clinic_admin') {
      // Clinic admins can only create doctors or staff for their own clinic
      if (!['doctor', 'staff'].includes(role)) {
        return res.status(403).json({ message: 'Clinic admins can only create doctor or staff users' });
      }
    } else if (req.user.role === 'super_admin') {
      // Super admin must provide clinicId for non-super_admin clinic roles
      if (['clinic_admin', 'doctor', 'staff'].includes(role)) {
...
// @desc    Get all clinic users (doctors and staff)
// @route   GET /api/users
// @access  Private (clinic_admin, staff)
export const getClinicUsers = async (req, res) => {
  try {
    ...
    const users = await User.find({ clinicId, role: { $in: ['doctor', 'staff', 'clinic_admin'] } }).select('-password');
    res.json({ users });
  } catch (error) {
