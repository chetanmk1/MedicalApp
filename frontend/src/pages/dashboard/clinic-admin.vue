<template>
  <div class="q-gutter-y-lg">
    <div class="row justify-between items-center">
      <div>
        <h1 class="text-h4 font-weight-bold text-slate-800 q-mb-none">{{ clinicInfo ? clinicInfo.name : 'Clinic Dashboard' }}</h1>
    ...
    <q-card flat bordered style="border-radius: 16px;">
      ...
      >
        ...
        <q-tab name="users" label="Doctors & Staff" icon="people" />
      ...
      <q-tab-panels v-model="tab" animated>
        ...
        <q-tab-panel name="bookings" class="q-gutter-y-md">
          <div class="row justify-between items-center q-mb-md">
            ...
            <q-btn flat icon="refresh" color="grey-6" label="Reload Ledger" @click="fetchAppointments" />
          </div>
...
const selectDocSchedule = async (doc) => {
  ...
  try {
    const data = await $api(`/schedules/doctor/${doc._id}`);
...
const saveSchedule = async () => {
  ...
  try {
    await $api(`/schedules/doctor/${selectedDocForSchedule.value._id}`, {
      method: 'PUT',
    ...
    $q.notify({
      type: 'negative',
      message: err._data?.message || 'Failed to save schedule'
    });
  } finally {
...
const submitReschedule = async () => {
  ...
  try {
    await $api(`/appointments/${rescheduleAppId.value}/reschedule`, {
      method: 'PATCH',
